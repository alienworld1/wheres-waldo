...
import { recordTime, saveUser } from '../apis/backend-apis';
...
const LeaderboardForm = forwardRef(
  ({ user, photoName }: LeaderboardFormProps, ref) => {
    const [name, setName] = useState<string>('');
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
      recordTime(user._id).catch(err =>
        setError(
    ...
    return (
      <dialog
        className="rounded-lg shadow-xl backdrop:bg-gray-800 backdrop:bg-opacity-50 absolute top-28"
        open={isOpen}
      >
...
export interface LeaderboardFormRef {
  open: () => void;
  close: () => void;
}
